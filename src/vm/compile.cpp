...
namespace ami::vm {
  std::unordered_map<node_type, std::expected<void, std::string>(compiler::*)(ami::node*)> compilers {
...
#define AMI_SIMPLE_CVT_TOKTYPE_TO_INSTR(op) case tok_type::op: ch.add(op_code::op); break;
  ...
  std::expected<void, std::string> compiler::bin_op(ami::node* n) {
    ...
    switch (it->op) {
      ...
      AMI_SIMPLE_CVT_TOKTYPE_TO_INSTR(sym_or)
      AMI_SIMPLE_CVT_TOKTYPE_TO_INSTR(sym_and)
